...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retraining pipeline triggered by drift metrics")
...
def fetch_latest_max_psi(engine) -> float:
...
def insert_run(engine, run_id: str, trigger_reason: str, trigger_value: float) -> None:
...
def finalize_run(engine, run_id: str, status: str, model_version: str | None, details: dict) -> None:
    with engine.begin() as conn:
        conn.execute(
            text(
                """
...
def main() -> None:
    args = parse_args()
    engine = get_engine()
    max_psi = fetch_latest_max_psi(engine)
    ...
    insert_run(engine, run_id, "psi_threshold", max_psi)
    ...
    try:
        ...
        print(json.dumps({"run_id": run_id, "result": result, "max_psi": max_psi}))
